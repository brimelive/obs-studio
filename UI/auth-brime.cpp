...
#define BRIME_AUTH_URL \
	"https://auth.brime.tv/authorize?response_type=code&client_id=cWzNPkYY3WJNNann531URZJLgPe0C5Op&redirect_uri=https://obsproject.com/app-auth/brime&scope=offline_access&audience=https://api.brime.tv/&state=2348273492374"
#define BRIME_TOKEN_URL "https://obs-mock-oauth.brime.workers.dev/"

#define BRIME_SCOPE_VERSION 1

static Auth::Def brimeDef = {"Brime Live", Auth::Type::OAuth_StreamKey};

/* ------------------------------------------------------------------------- */

BrimeAuth::BrimeAuth(const Def &d) : OAuthStreamKey(d)
{
	if (!cef)
		return;

	cef->add_popup_whitelist_url(
		"https://brime.tv/popout/frankerfacez/chat?ffz-settings",
		this);
	...
	connect(&uiLoadTimer, &QTimer::timeout, this,
		&BrimeAuth::TryLoadSecondaryUIPanes);
}

bool BrimeAuth::MakeApiRequest(const char *path, Json &json_out)
{
	std::string client_id = BRIME_CLIENTID;
	deobfuscate_str(&client_id[0], BRIME_HASH);

	std::string url = "https://api.brime.tv/v1/";
	url += std::string(path);
	...
	if (error_code == 403) {
		OBSMessageBox::warning(OBSBasic::Get(),
				       Str("BrimeAuth.TwoFactorFail.Title"),
				       Str("BrimeAuth.TwoFactorFail.Text"),
				       true);
		blog(LOG_WARNING, "%s: %s", __FUNCTION__,
		     "Got 403 from Brime, user probably does not "
...
bool BrimeAuth::GetChannelInfo()
try {
	std::string client_id = BRIME_CLIENTID;
	deobfuscate_str(&client_id[0], BRIME_HASH);

	if (!GetToken(BRIME_TOKEN_URL, client_id, BRIME_SCOPE_VERSION))
		return false;
	if (token.empty())
		return false;
	if (!key_.empty())
	...
	Json json;
	bool success = MakeApiRequest("account/me", json);

	if (!success)
		return false;

	name = json["username"].string_value();

	std::string path = "account/streamkey";
...
localStorage.setItem('bttv_clickBrimeEmotes', true);\
...
void BrimeAuth::LoadUI()
{
	...
	/* Brime panels require a UUID, it does not actually need to be unique,
...
void BrimeAuth::LoadSecondaryUIPanes()
{
	OBSBasic *main = OBSBasic::Get();
	...
	url = "https://dashboard.brime.tv/popout/u/";
	url += name;
	url += "/stream-manager/edit-stream-info";

	info.reset(new BrowserDock());
	info->setObjectName("brimeInfo");
	info->resize(300, 650);
	...
	url = "https://www.brime.tv/popout/";
	url += name;
	url += "/dashboard/live/stats";

	stat.reset(new BrowserDock());
	stat->setObjectName("brimeStats");
	stat->resize(200, 250);
	stat->setMinimumSize(200, 150);
	stat->setWindowTitle(QTStr("BrimeAuth.Stats"));
	...
	feed->setObjectName("brimeFeed");
	feed->resize(300, 650);
	feed->setMinimumSize(200, 300);
	feed->setWindowTitle(QTStr("BrimeAuth.Feed"));
...
/* Brime.tv has an OAuth for itself.  If we try to load multiple panel pages
 ...
 * subsequent panels should only be loaded once we know that Brime has auth'ed
 * itself (if the cookie "auth-token" exists for brime.tv).
 *
 * This is annoying to deal with. */
void BrimeAuth::TryLoadSecondaryUIPanes()
{
	QPointer<BrimeAuth> this_ = this;
	...
	panel_cookies->CheckForCookie("https://www.brime.tv", "auth-token",
				      cb);
}

bool BrimeAuth::RetryLogin()
{
	OAuthLogin login(OBSBasic::Get(), BRIME_AUTH_URL, false);
	if (login.exec() == QDialog::Rejected) {
		return false;
	}

	std::shared_ptr<BrimeAuth> auth =
		std::make_shared<BrimeAuth>(brimeDef);
	std::string client_id = BRIME_CLIENTID;
	deobfuscate_str(&client_id[0], BRIME_HASH);

	return GetToken(BRIME_TOKEN_URL, client_id, BRIME_SCOPE_VERSION,
			QT_TO_UTF8(login.GetCode()), true);
}

std::shared_ptr<Auth> BrimeAuth::Login(QWidget *parent, const std::string &)
{
	OAuthLogin login(parent, BRIME_AUTH_URL, false);
	if (login.exec() == QDialog::Rejected) {
		return nullptr;
	}
	...
	if (!auth->GetToken(BRIME_TOKEN_URL, client_id, BRIME_SCOPE_VERSION,
...
static std::shared_ptr<Auth> CreateBrimeAuth()
{
	return std::make_shared<BrimeAuth>(brimeDef);
}

static void DeleteCookies()
{
	if (panel_cookies)
		panel_cookies->DeleteCookies("brime.tv", std::string());
}

void RegisterBrimeAuth()
{
	OAuth::RegisterOAuth(brimeDef, CreateBrimeAuth, BrimeAuth::Login,
			     DeleteCookies);
}
